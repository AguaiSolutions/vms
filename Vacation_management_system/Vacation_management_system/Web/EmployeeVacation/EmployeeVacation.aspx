...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            validate();
            $('#<%= txtRejectreason.ClientID %>').keyup(validate);
         });

        function validate() {
            $('#<%= Button5.ClientID %>').addClass('btn');
             if ($('#<%= txtRejectreason.ClientID %>').val().length > 0) {
                 $('#<%= Button5.ClientID %>').prop("disabled", false);
                $('#<%= Button5.ClientID %>').addClass('btn-primary');

                 <%--$('#<%= btnRole.ClientID %>').css('backgroundColor', 'blue');--%>
            }
            else {
                $('#<%= Button5.ClientID %>').prop("disabled", true);
                $('#<%= Button5.ClientID %>').removeClass('btn-primary');
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $('.cancel').click(function () {
                $('#<%= txtRejectreason.ClientID %>').val('');
                var x = document.getElementById('<%=txtRejectreason.ClientID%>').value;
                if (x == "") {
                    $('#<%= Button5.ClientID %>').prop("disabled", true);
                    $('#<%= Button5.ClientID %>').removeClass('btn-primary');
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                ...
                                <div class="form-group form-inline">
                                    <asp:Button ID="btnApprovevacation" CssClass="btn btn-primary" runat="server" Text="Approve Vacation" Enabled='<%# GridView1.Rows.Count!=0 %>' OnClick="btnApprove_Click" />

                                    <input type="button" id="btnRejectvacation" class="btn btn-primary" runat="server" name="button" value="Reject Vacation" disabled='<%# GridView1.Rows.Count==0 %>' onclick="openModal();" />
                                </div>
                                <div>
                                    ...
                                    <asp:GridView ID="GridView1" runat="server" OnPageIndexChanging="grdview1_PageIndexChanging" AutoGenerateColumns="False" AllowPaging="true" PageSize="10" class="table table-bordered bg-danger"  OnRowDataBound="GridView1_RowDataBound"  DataKeyNames="id,emp_id" CellPadding="4" ForeColor="#333333" EmptyDataText="No records found" ShowHeaderWhenEmpty="true" GridLines="None">
                                ...
                                <div class="form-group form-inline" style="margin-top: 30px;">
                                    ...
                                    <input type="button" id="btnRejectvacation1" class="btn btn-primary" name="button" runat="server" value="Reject Vacation" onclick="openModal();" />
                                </div>
                                ...
                                <div class="panel panel-info">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <div class="center-block">
                                            ...
                                            <asp:Button ID="Button4" runat="server" CssClass="cancel btn btn-primary" data-dismiss="modal" Font-Size="Large" Text="Back" />
