...
namespace Vacation_management_system.Web.Common.Class
{
    public class Email
    {
        ...
        public bool ForgotPassword(string Employee_name, string user_email, string url)
        {
            try
            {
                var message = new MailMessage();
                ...
                GetMailBodyForNewUser("~/Web/Common/MailTemplate/ForgotPassword.xml", out subject, out body);
                body = body.Replace("$$Employee Name$$", Employee_name); 
                body = body.Replace("$$host$$", url);
                message.Subject = subject;
                message.Body = body;
                SmtpClient smtpClient = new SmtpClient();
